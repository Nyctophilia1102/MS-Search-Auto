...
def perform_search(driver, num_searches=10):
    ...
    for i in range(num_searches):
        ...
        # Random delay between 6 and 8 seconds
...
if __name__ == "__main__":
    ...
    while attempts < max_retries:
        try:
            print(f"Starting Edge browser, attempt {attempts + 1}")
            driver = setup_driver()
            perform_search(driver, num_searches=30)
            break  # Thoát khỏi vòng lặp nếu chạy thành công
        except Exception as e:
            print(f"Error occurred: {e}")
            attempts += 1
            print("Restarting browser after crash or failure...")
            time.sleep(5)  # Đợi vài giây trước khi restart
        finally:
            try:
                driver.quit()
            except:
